...
private:
	...
	//作成
	bool CreatePeraResourcesAndView(void);

	ComPtr<ID3D12Resource> _bokehParamResource;
	//ボケに関するバッファを作り中にボケパラメータを代入する
	bool CreateBokehParamResource(void);
	...
	bool CreatePeraVertex(void);
	bool CreatePeraPipeline(void);

public:
	Dx12Wrapper(HWND hwnd);
	~Dx12Wrapper();

	ID3D12Device* Device(void)
	{
		return _dev.Get();
	}
	ID3D12GraphicsCommandList* CmdList(void)
	{
		return _cmdList.Get();
	}
	...
	//ペラポリゴンへの描画準備
	bool PreDrawToPera1(void);
	//ペラポリゴンへの描画後処理
	void PostDrawToPera1(void);
